<script setup>
  ...
  const { floatingStyles } = useFloating(durationFloatingReference, floatingComponent, {
    ...
    <div
      ...
      <div class="task-content">
        ...
        <div v-if="tags.length" v-auto-animate class="task-meta">
          <span v-for="(tag, index) in tags" :key="index" class="task-tag" :class="`tag-${getTagColor(tag)}`">
            <Tag size="12" />
            {{ tag }}
