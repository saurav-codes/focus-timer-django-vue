<script setup>
  ...
  import { Undo2 as Undo, X, Check, AlertCircle, Info } from 'lucide-vue-next'
...
<template>
  <div class="snackbar-container">
    <TransitionGroup name="snackbar">
      <div v-for="item in snackbarItems" :key="item.id" class="snackbar">
        <div class="snackbar-content">
          <span class="message">{{ item.message }}</span>
          <button v-if="item.buttonAction" class="action-button" @click="() => executeAction(item)">
            <component :is="getActionIcon(item.actionText)" class="action-icon" :size="16" />
            {{ item.actionText }}
          </button>
        </div>
      </div>
