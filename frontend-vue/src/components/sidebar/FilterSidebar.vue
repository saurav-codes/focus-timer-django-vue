...
<template>
  <div v-if="isVisible" class="filter-sidebar-overlay" @click="closeSidebar">
    <div class="filter-sidebar" @click.stop>
      ...
      <p class="filter-description">Filter task data by projects and tags.</p>

      <!-- Search input -->
      <div class="search-container">
        <SearchIcon size="18" class="search-icon" />
        <input v-model="searchQuery" type="text" placeholder="Search for label" class="search-input" />
      </div>
      ...
      <div class="filter-section">
        ...
        <div class="filter-items">
          <div
            ...
            <div class="checkbox-wrapper">
              <input type="checkbox" :checked="selectedTags.includes(tag.name)" @click.stop />
              <span class="checkmark" />
            </div>
            <span>{{ tag.name }}</span>
          </div>
