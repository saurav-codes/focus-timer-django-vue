...
def test_get_tasks_with_filter(
    authenticated_client, task, kanban_task, calendar_task, completed_task
):
...
def test_update_task_order_bulk(authenticated_client, task, kanban_task):
    ...
    data = {
        "action": "update_order",
        "tasks": [{"id": kanban_task.id}, {"id": task.id}],
    }
...
def test_task_access_control(api_client, authenticated_client, authenticated_user):
    ...
    second_user = User.objects.create_user(
        email="seconduser@example.com", password="securepass123"
    )
